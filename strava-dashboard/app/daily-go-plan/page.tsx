  ...
  return (
    <div className={`rounded-2xl border p-5 ${bg}`}>
      <div className="flex items-start gap-3">
        <div className={`mt-0.5 p-2 rounded-xl ${isDark ? "bg-white/10" : "bg-white/60"}`}>
      ...
      {targetKm != null && (
        <div className={`mt-4 pt-4 ${isDark ? "border-t border-white/10" : "border-t border-stone-200"}`}>
          <div className={`h-1.5 rounded-full overflow-hidden mb-2.5 ${isDark ? "bg-white/10" : "bg-stone-100"}`}>
            <div
          ...
          <div className="text-center">
            <span className={`text-xl font-light tabular-nums ${isDark ? "text-white" : "text-stone-800"}`}>
              {doneKm.toFixed(1)}
            </span>
            <span className={`mx-1 text-sm ${isDark ? "text-white/30" : "text-stone-300"}`}>/</span>
            <span className={`text-base font-light tabular-nums ${isDark ? "text-white/50" : "text-stone-400"}`}>
  ...
  return (
    <main className="min-h-screen bg-[#F7F3EE] flex flex-col items-center justify-start pt-safe">
      <div className="w-full max-w-md px-5 pb-10 pt-8 flex flex-col gap-4">
        ...
        {/* Briefing + weekly progress (merged) */}
        {briefing ? (
          <BriefingCard briefing={briefing} targetKm={targetKm} doneKm={doneKm} />
        ) : (
