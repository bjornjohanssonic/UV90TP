...
const DB_PATH =
  process.env.STRAVA_DB_PATH ??
  path.resolve(__dirname, "../../strava-dashboard/strava.db");
...
function getAthleteId(): string | undefined {
  const row = db.prepare("SELECT strava_athlete_id FROM users LIMIT 1").get() as
...
server.tool(
  ...
  async ({ limit, type }) => {
    ...
    const lines = rows.map((r, i) => {
      ...
      const dist = r.distance ? metersToKm(r.distance) : "–";
      const time = r.moving_time ? secondsToHms(r.moving_time) : "–";
      const pace = r.average_speed ? mpsToMinPerKm(r.average_speed) : "–";
      ...
      const suffer = r.suffer_score ? `suffer=${Math.round(r.suffer_score)}` : "";
      const battery =
    ...
    return {
      content: [
        {
          type: "text",
          text: `## Senaste ${rows.length} aktiviteter (typ: ${type})\n\n${lines.join("\n")}`,
        },
      ],
    };
  }
...
server.tool(
  ...
  async ({ weeks }) => {
    ...
    const rows = db
      .prepare(
        `SELECT
         ...
         FROM activities
         WHERE type = 'Run'
           AND (athlete_id = ? OR athlete_id IS NULL)
           AND start_date >= date('now', ?)
         GROUP BY week
         ORDER BY week DESC`
      )
      .all(athleteId ?? "", `-${weeks * 7} days`) as any[];
    ...
    const lines = rows.map((r) => {
      const dist = r.total_dist ? metersToKm(r.total_dist) : "0 km";
      const time = r.total_time ? secondsToHms(r.total_time) : "–";
      const hr = r.avg_hr ? `${Math.round(r.avg_hr)} bpm` : "–";
      const elev = r.total_elev ? `${Math.round(r.total_elev)}m ↑` : "–";
      const suffer = r.total_suffer ? `, suffer: ${Math.round(r.total_suffer)}` : "";
    ...
    return {
      content: [
        {
          type: "text",
          text: `## Veckosummering — senaste ${weeks} veckor\n\n${lines.join("\n")}`,
        },
      ],
    };
  }
...
server.tool(
  ...
  async () => {
    ...
    const plan = db
      .prepare(
        `SELECT * FROM training_plans
         WHERE active = 1 AND (athlete_id = ? OR athlete_id IS NULL)
    ...
    const weeks = db
      .prepare(
        `SELECT tw.*,
                (SELECT SUM(a.distance) / 1000.0
                 FROM activities a
                 WHERE a.type = 'Run'
                   AND (a.athlete_id = ? OR a.athlete_id IS NULL)
      ...
      .all(athleteId ?? "", athleteId ?? "", plan.id) as any[];
    ...
    const currentWeek = weeks.find((w) => {
      ...
      return todayStr >= w.start_date && todayStr <= end.toISOString().slice(0, 10);
    });
    ...
    const currentInfo = currentWeek
      ? [
          ...
          `Mål: ${currentWeek.target_volume_km.toFixed(1)} km | Faktisk: ${(currentWeek.actual_km ?? 0).toFixed(1)} km | Löpturer: ${currentWeek.run_count ?? 0}`,
    ...
    const weekLines = weeks.map((w) => {
      const actual = w.actual_km ?? 0;
      const pct = w.target_volume_km > 0 ? Math.round((actual / w.target_volume_km) * 100) : 0;
      const isCurrent = currentWeek?.week_number === w.week_number ? " ◄" : "";
      return `v${String(w.week_number).padStart(2, "0")} [${w.phase.padEnd(8)}] mål: ${w.target_volume_km.toFixed(1).padStart(5)} km | faktisk: ${actual.toFixed(1).padStart(5)} km (${pct}%) | LR: ${w.long_run_km} km${w.back_to_back ? " B2B" : ""}${isCurrent}`;
    });
    ...
    return {
      content: [
        {
          type: "text",
          text: header + currentInfo + "### Alla veckor\n\n" + weekLines.join("\n"),
        },
      ],
    };
  }
...
server.tool(
  ...
  async () => {
    const athleteId = getAthleteId();
    const today = new Date();
    const dayOfWeek = today.getDay(); // 0=sön
    ...
    const activities = db
      .prepare(
        `SELECT name, distance, moving_time, average_speed, average_heartrate,
      ...
      .all(athleteId ?? "", mondayStr, sundayStr) as any[];

    const runs = activities.filter((a) => a.type === "Run");
    const gym = activities.filter((a) => a.type === "WeightTraining");

    const totalKm = runs.reduce((s, r) => s + (r.distance ?? 0), 0) / 1000;
    const totalTime = runs.reduce((s, r) => s + (r.moving_time ?? 0), 0);

    // Aktiv plan + vecka
    const plan = db
      .prepare(
        `SELECT tp.*, tw.week_number, tw.target_volume_km, tw.long_run_km,
                tw.back_to_back, tw.phase
         FROM training_plans tp
         JOIN training_weeks tw ON tw.plan_id = tp.id
         WHERE tp.active = 1 AND (tp.athlete_id = ? OR tp.athlete_id IS NULL)
           AND tw.start_date = ?
         LIMIT 1`
      )
      .get(athleteId ?? "", mondayStr) as any;
    ...
    if (gym.length > 0) {
      lines.push(`**Gym:** ${gym.length} pass`);
    }
    ...
    if (plan) {
      const remaining = Math.max(0, plan.target_volume_km - totalKm);
      const pct = plan.target_volume_km > 0 ? Math.round((totalKm / plan.target_volume_km) * 100) : 0;
      lines.push("");
      lines.push(`**Planvecka v${plan.week_number} (${plan.phase}):**`);
      lines.push(`  Mål: ${plan.target_volume_km.toFixed(1)} km | Gjort: ${totalKm.toFixed(1)} km (${pct}%)`);
      lines.push(`  Återstår: ${remaining.toFixed(1)} km | Långpass mål: ${plan.long_run_km} km`);
...
server.tool(
  ...
  async ({ top_n }) => {
    const athleteId = getAthleteId();
    const base = `WHERE type = 'Run' AND (athlete_id = ? OR athlete_id IS NULL)`;
    const p = athleteId ?? "";

    const longestRuns = db
      .prepare(
        `SELECT name, distance, start_date, moving_time FROM activities
         ${base} AND distance IS NOT NULL ORDER BY distance DESC LIMIT ?`
      )
      .all(p, top_n) as any[];

    const fastestPace = db
      .prepare(
        `SELECT name, average_speed, distance, start_date FROM activities
         ${base} AND average_speed IS NOT NULL AND distance >= 3000
         ORDER BY average_speed DESC LIMIT ?`
      )
      .all(p, top_n) as any[];

    const mostElevation = db
      .prepare(
        `SELECT name, total_elevation_gain, distance, start_date FROM activities
         ${base} AND total_elevation_gain IS NOT NULL ORDER BY total_elevation_gain DESC LIMIT ?`
      )
      .all(p, top_n) as any[];

    const longestTime = db
      .prepare(
        `SELECT name, moving_time, distance, start_date FROM activities
         ${base} AND moving_time IS NOT NULL ORDER BY moving_time DESC LIMIT ?`
      )
      .all(p, top_n) as any[];

    const highestSuffer = db
      .prepare(
        `SELECT name, suffer_score, distance, start_date FROM activities
         ${base} AND suffer_score IS NOT NULL ORDER BY suffer_score DESC LIMIT ?`
      )
      .all(p, top_n) as any[];

    const fmt = (rows: any[], fn: (r: any) => string) =>
      rows.map((r, i) => `  ${i + 1}. ${r.name} (${r.start_date?.slice(0, 10)}): ${fn(r)}`).join("\n");

    const text = [
      "## Personliga rekord",
      "",
      `### Längsta löpturer`,
      fmt(longestRuns, (r) => `${(r.distance / 1000).toFixed(1)} km (${secondsToHms(r.moving_time)})`),
      "",
      `### Snabbaste tempo (min/km)`,
      fmt(fastestPace, (r) => `${mpsToMinPerKm(r.average_speed)} på ${(r.distance / 1000).toFixed(1)} km`),
      "",
      `### Mest elevation`,
      fmt(mostElevation, (r) => `${Math.round(r.total_elevation_gain)} m ↑ (${(r.distance / 1000).toFixed(1)} km)`),
      "",
      `### Längsta tid`,
      fmt(longestTime, (r) => `${secondsToHms(r.moving_time)} (${(r.distance / 1000).toFixed(1)} km)`),
      "",
      `### Högst suffer score`,
      fmt(highestSuffer, (r) => `${Math.round(r.suffer_score)} (${(r.distance / 1000).toFixed(1)} km)`),
    ].join("\n");
...
server.tool(
  ...
  async ({ from_date, to_date, type, min_distance_km, max_distance_km, name_contains, limit }) => {
    const athleteId = getAthleteId();
    const conditions: string[] = ["(athlete_id = ? OR athlete_id IS NULL)"];
    const params: (string | number)[] = [athleteId ?? ""];

    if (from_date) { conditions.push("date(start_date) >= ?"); params.push(from_date); }
    if (to_date) { conditions.push("date(start_date) <= ?"); params.push(to_date); }
    if (type) { conditions.push("type = ?"); params.push(type); }
    if (min_distance_km) { conditions.push("distance >= ?"); params.push(min_distance_km * 1000); }
    if (max_distance_km) { conditions.push("distance <= ?"); params.push(max_distance_km * 1000); }
    if (name_contains) { conditions.push("name LIKE ?"); params.push(`%${name_contains}%`); }

    params.push(limit);

    const rows = db
      .prepare(
        `SELECT name, type, distance, moving_time, average_speed,
...
server.tool(
  ...
  async () => {
    ...
    const rows = db
      .prepare(
        `SELECT date(start_date) AS day, SUM(distance) / 1000.0 AS km
         FROM activities
         WHERE type = 'Run'
           AND (athlete_id = ? OR athlete_id IS NULL)
      ...
      .all(athleteId ?? "", today, today) as { day: string; km: number }[];
    ...
    const acuteAvg = acute; // 7-day sum
    const chronicAvg = chronic / 4; // 28-day avg weekly
    ...
    const text = [
      ...
      "",
      `Akut (senaste 7 dagarna): ${acuteAvg.toFixed(1)} km`,
...
server.tool(
  ...
  async ({ activity_name, date }) => {
    const athleteId = getAthleteId();
    const params: (string | number)[] = [athleteId ?? "", `%${activity_name}%`];
    if (date) params.push(date);
    params.push(1);

    const row = db
      .prepare(
        `SELECT name, start_date, distance, splits, average_speed
         FROM activities
         WHERE (athlete_id = ? OR athlete_id IS NULL)
           AND name LIKE ?
    ...
    if (!row) {
      return { content: [{ type: "text", text: "Hittade ingen matchande aktivitet." }] };
    }

    if (!row.splits) {
      return {
        content: [{ type: "text", text: `${row.name} (${row.start_date?.slice(0, 10)}) — inga split-data lagrade.` }],
      };
    }
      ...
      content: [
        {
          type: "text",
          text: [
            `## Splittar: ${row.name} (${row.start_date?.slice(0, 10)})`,
            `Distans: ${(row.distance / 1000).toFixed(1)} km | Snitt: ${mpsToMinPerKm(row.average_speed)}`,
            "",
            ...splitLines,
          ].join("\n"),
        },
...
async function main() {
  ...
  console.error("Strava MCP server igång — lyssnar på stdio");
}
